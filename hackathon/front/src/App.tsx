...
function App() {
  return (
    <main className="min-h-screen bg-[#eef2f6] text-slate-950">
      <div className="mx-auto flex min-h-screen w-full max-w-[430px] flex-col bg-[#fbfcfe] shadow-xl shadow-slate-300/60">
        <header className="sticky top-0 z-10 border-b border-slate-200/80 bg-[#fbfcfe]/95 px-5 py-4 backdrop-blur">
          <div className="flex items-center justify-between">
            <Link to="/" className="flex flex-col">
              <span className="text-xl font-black tracking-normal">ShoeFit</span>
              <span className="text-xs font-semibold text-slate-500">AI size match</span>
            </Link>
            <Link
              to="/measure"
    ...
    <section className="space-y-6 px-5 py-5">
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Link to="/recommendations" className="text-sm font-bold text-cyan-700">
            더보기
          </Link>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {featuredProducts.map((product) => (
            <article key={product.id} className="rounded-[8px] border border-slate-200 bg-white p-3">
              <div className="aspect-square rounded-[8px] bg-gradient-to-br from-slate-100 to-cyan-100" />
              <p className="mt-3 text-xs font-bold text-slate-500">{product.brand}</p>
...
function MeasurePage() {
  const steps = ['측정 동의', '촬영 이미지 업로드', '품질 검증', 'AI 측정 분석', '사이즈 추천']

  return (
    <section className="space-y-5 px-5 py-5">
      <div>
        <p className="text-sm font-bold text-cyan-700">Foot measurement</p>
        <h1 className="mt-1 text-2xl font-black tracking-normal">발 촬영으로 추천 사이즈 받기</h1>
      </div>
      ...
      <ol className="space-y-3">
        {steps.map((step, index) => (
          <li key={step} className="flex items-center gap-3 rounded-[8px] border border-slate-200 bg-white p-4">
  ...
  return (
    <section className="space-y-4 px-5 py-5">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-black tracking-normal">추천 사이즈</h1>
      </div>
      ...
      <div className="rounded-[8px] bg-white p-5 shadow-sm shadow-slate-200">
        ...
        <p className="mt-4 text-sm leading-6 text-slate-600">
          로그인 후 발 프로필과 상품 데이터를 기준으로 백엔드 추천 API와 연결합니다.
        </p>
      </div>
    </section>
  )
